...
namespace TransformRelated
{
	...
	public:
		...
		// Tests that rotating a vector is equal to multiplying it with the rotation matrix
		...
		// Tests that setting all rotations to 90 degrees, and then transforming a point, does not change its position IF it only lies on the X axis
		//TEST_METHOD(X_Axis_Point_Not_Affected_When_XYZ90)
		//{
		//	Quaternion a(Vector3d(0, 45, 0));
		//
		//	Vector3d point(19, 0, 0);
		//
		//	std::wstringstream wss;
		//	wss << std::endl
		//		<< L"Expected: " << point << std::endl
		//		<< L"Actual: " << point * a.ToRotationMatrix() << std::endl;
		//
		//	Assert::IsTrue(point.Similar(point * a.ToRotationMatrix()), wss.str().c_str());
		//
		//	return;
		//}
